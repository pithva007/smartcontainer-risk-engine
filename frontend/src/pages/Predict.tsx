...
const fields: { key: string; label: string; type: string; required?: boolean }[] = [
    ...
    { key: 'Importer_ID', label: 'Importer ID', type: 'text', required: true },
    { key: 'Exporter_ID', label: 'Exporter ID', type: 'text', required: true },
    { key: 'Declared_Value', label: 'Declared Value ($)', type: 'number', required: true },
    { key: 'Declared_Weight', label: 'Declared Weight (kg)', type: 'number', required: true },
    { key: 'Measured_Weight', label: 'Measured Weight (kg)', type: 'number', required: true },
    { key: 'Dwell_Time_Hours', label: 'Dwell Time (Hours)', type: 'number', required: true },
];
...
export default function Predict() {
    ...
    const mutation = useMutation({
        ...
        onSuccess: (data) => {
            setResult(data);
            toast.success(
                (data.auto_escalated_by_importer_history || data.auto_escalated_by_new_trader_rule)
    ...
    const isFormValid = () => {
        return fields.every(f => !f.required || (form[f.key] !== undefined && form[f.key] !== ''));
    };
    ...
    return (
        <div className="space-y-8 pb-8 max-w-5xl mx-auto">
            <div>
                <h1 className="text-2xl font-bold text-foreground">Container Risk Assessment</h1>
                <p className="text-sm text-foreground/60 mt-1">Leverage machine learning to identify high-risk containers based on shipment characteristics.</p>
                <p className="text-xs text-foreground/40 mt-1">Provide container information to generate an AI-powered risk assessment.</p>
            </div>

            <form onSubmit={handleSubmit} className="bg-card border border-border rounded-xl p-6 shadow-sm">
                <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide mb-4">Container Shipment Details</h2>
                ...
                <div className="flex justify-end mt-6">
                    ...
                    >
                        {mutation.isPending ? <><Loader2 className="w-4 h-4 animate-spin" /> Analyzing...</> : <><Crosshair className="w-4 h-4" /> Analyze Risk</>}
                    </button>
                </div>
            </form>
            ...
            {result && (
                <div className="space-y-4">
                    ...
                    {(result.auto_escalated_by_importer_history || result.auto_escalated_by_new_trader_rule) && (
                    ...
                    <div className="bg-card border border-border rounded-xl p-6 shadow-sm space-y-5">
                        ...
                        {result.importer_stats && (
                            <div className="flex flex-col sm:flex-row gap-3">
                                <div className="flex items-center flex-wrap gap-3 p-3 bg-foreground/5 rounded-lg border border-border text-xs flex-1">
                                ...
                                {/* EXPORTER STATS */}
                                {result.exporter_historical_shipment_count !== undefined && (
                                    <div className="flex items-center flex-wrap gap-3 p-3 bg-foreground/5 rounded-lg border border-border text-xs flex-1">
                                        <span className="flex items-center gap-1.5 text-muted-foreground">
                                            <History className="w-3.5 h-3.5" />
                                            <span className="font-medium">Exporter History:</span>
                                        </span>
                                        <span className="text-foreground">
                                            <span className={cn('font-semibold', result.auto_escalated_by_new_trader_rule ? 'text-red-500' : 'text-foreground')}>{result.exporter_historical_shipment_count}</span> shipments
                                        </span>
                                        {result.auto_escalated_by_new_trader_rule && (
                                            <span className="text-red-500">
                                                (New Trader threshold missed)
                                            </span>
                                        )}
                                    </div>
                                )}
